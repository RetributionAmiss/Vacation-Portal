/**
 * Vacation Portal V4.3.0 push notifications.
 *
...
function testOneSignalSetup() {
  ...
  if (!status.appIdConfigured) {
    throw new Error(
      'Enter the OneSignal App ID in Trip Settings first.'
    );
  }

  if (!status.apiKeyConfigured) {
    throw new Error(
      'Use Vacation Portal → Set OneSignal API key first.'
    );
  }

  safeUiAlert_(
    'OneSignal setup',
    'OneSignal App ID and API key are configured. ' +
      'Install the PWA on a test device and enable notifications before sending a test.'
  );

  return status;
}

function assertJustinTraveler_(travelerId) {
  const traveler = readSheet_('Travelers').find(function(row) {
    return String(row['Traveler ID'] || '') === String(travelerId || '');
  });
  ...
  if (
    ...
    throw new Error('Only Justin can send portal notifications.');
  }
...
function sendPortalPushNotification(values) {
  values = values || {};
  assertJustinTraveler_(values.senderTravelerId);
  ...
  if (!appId) {
    throw new Error('OneSignal App ID is not configured in Trip Settings.');
  }
  if (!apiKey) {
    throw new Error(
      'OneSignal API key is not configured. ' +
      'Use the Vacation Portal spreadsheet menu.'
    );
  }
  ...
  if (!targetIds.length) {
    throw new Error('No travelers are selected for this notification.');
  }
