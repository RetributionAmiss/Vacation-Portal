function saveTraveler(values) {
  ...
  const isNew = !String(values.id || '').trim();
  const id = String(values.id || '').trim() || uid_('TRAV');

  const travelerType =
    String(values.travelerType || 'Adult').trim() === 'Child'
      ? 'Child'
      : 'Adult';

  const parentId = travelerType === 'Child'
    ? String(values.parentGuardianId || '').trim()
    : '';
  ...
  if (parentId) {
    ...
    if (!parent) {
      throw new Error('The selected parent or guardian was not found.');
    }
  ...
  const record = {
    ...
    'Price Cap': travelerType === 'Adult'
      ? Math.max(0, Number(values.priceCap || 0))
      : 0,
    'Cost %': travelerType === 'Adult'
      ...
      : 0,
    'Pay More': travelerType === 'Adult' && Boolean(values.payMore)
      ? 'Yes'
      : 'No',
    'Willing to Share Room': travelerType === 'Adult' && Boolean(values.shareRoom)
      ? 'Yes'
      : 'No',
    'Home Location': String(values.homeLocation || '').trim(),
  ...
  if (idIndex < 0) {
    throw new Error('Traveler ID column was not found.');
  }
  ...
  if (!isNew) {
    ...
    if (!rowNumber) {
      throw new Error('The selected traveler could not be found.');
    }
  }
...
function migrateTravelerTypes_() {
  ...
  if (typeIndex < 0 || parentIndex < 0 || adultIndex < 0 || childIndex < 0) {
    return;
  }
  ...
  for (let rowIndex = 1; rowIndex < values.length; rowIndex++) {
    ...
    sheet.getRange(rowNumber, adultIndex + 1)
      .setValue(inferredType === 'Adult' ? 1 : 0);
    sheet.getRange(rowNumber, childIndex + 1)
      .setValue(inferredType === 'Child' ? 1 : 0);
