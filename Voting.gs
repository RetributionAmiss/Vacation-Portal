  ...
  return withPortalMutationLock_(function() {
    ...
    for (let row = 1; row < valuesGrid.length; row++) {
      if (
        ...
        createdAt = createdAtIndex >= 0 ? valuesGrid[row][createdAtIndex] : null;
        break;
      }
    }
...
function toggleFavorite(cabinId, travelerId) {
  return withPortalMutationLock_(function() {
    const existing = readSheet_('Favorites').find(f =>
      f['Cabin ID'] === cabinId && f['Traveler ID'] === travelerId
    );
...
function saveComment(values) {
  ...
  const scope = 'rental-comment-' + cabinId + '-' + travelerId;
