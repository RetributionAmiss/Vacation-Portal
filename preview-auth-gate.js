...
function ensureShadowBadge_(){
  ...
  badge.textContent='Alpha2.28 · Sheets visible authority · Shadow waiting for authenticated check…';
...
function setShadowBadge_(status,legacyId,mismatchCount){
  ...
  if(state==='mismatch'){
    badge.textContent='Alpha2.28 · Sheets visible authority · Supabase shadow MISMATCH'+(id?' · '+id:'')+' · '+Number(mismatchCount||0)+' field(s)';
    return;
  }
...
function ensurePreviewGate_(){
  ...
  gate.innerHTML=`
    <section class="preview-auth-card" role="dialog" aria-modal="true" aria-labelledby="previewAuthTitle">
      <p class="eyebrow">ALPHA2.28 PREVIEW GATE</p>
      <h2 id="previewAuthTitle">Supabase sign-in required before shadow testing</h2>
      <p>The finalized rental must remain visibly Sheets-authoritative. This preview only performs an authenticated Supabase read in the background and compares it with the selected Sheet cabin.</p>
...
function reloadPortalFrameForAuth_(gate,status){
  ...
  frame.addEventListener('load',()=>{
    status.textContent='Supabase session + active trip membership confirmed. Waiting for finalized-rental shadow diagnostics…';
    status.className='preview-auth-ok';
    authReady=true;
