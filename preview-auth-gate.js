...
function ensurePreviewGate_(){
  ...
  gate.innerHTML=`
    <section class="preview-auth-card" role="dialog" aria-modal="true" aria-labelledby="previewAuthTitle">
      <p class="eyebrow">ALPHA2.26 MEALS PREVIEW GATE</p>
      <h2 id="previewAuthTitle">Supabase sign-in required before testing</h2>
      <p>This preview intentionally falls back to Sheets when no Supabase browser session exists. Testing while signed out does <strong>not</strong> exercise the new Meals-record primary read/write path.</p>
...
function reloadPortalFrameForAuth_(gate,status){
  ...
  status.textContent='Supabase session confirmed. Restarting the portal in Meals primary mode…';
  ...
  frame.addEventListener('load',()=>{
    status.textContent='Supabase session + active trip membership confirmed. Alpha2.26 Meals preview is ready.';
    status.className='preview-auth-ok';
    authReady=true;
...
async function checkPreviewAuth_(interactive=false){
  ...
  try{
    ...
    if(!session||!session.user){
      ...
      status.textContent='Signed out. Sign in before testing Meals records.';
      status.className='preview-auth-warn';
