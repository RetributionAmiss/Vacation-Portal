...
function child(){
  ...
  const ctx=vm.createContext({window:{top,addEventListener:(name,fn)=>listeners[name]=fn},setInterval:fn=>tick=fn,setTimeout:fn=>{timers.set(++id,fn);return id;},clearTimeout:id=>timers.delete(id),google:{script:{get run(){let ok,no;const c={withSuccessHandler(f){ok=f;return c;},withFailureHandler(f){no=f;return c;},getPaymentsBudgetFreshData(){calls.push({ok,no});}};return c;}}}});
...
(async()=>{
  ...
  let c=child();let p=c.tick();c.response(c.sent[0],{enabled:true,canSync:true});await flush();assert.equal(c.calls.length,1);c.calls[0].ok(source);await flush();assert.equal(c.sent[1].source.budget[0].Amount,12.34,'Only persisted source may be copied');c.response(c.sent[1],{status:'match',count:1});await p;assert.equal(vm.runInContext('DATA.budget[0].Amount',c.ctx),999,'Shadow sync must not replace UI or optimistic data');
  ...
  c=child();p=c.tick();c.response(c.sent[0],{enabled:true,canSync:true});await flush();vm.runInContext('DATA={deferredLoaded:true,budget:[]}',c.ctx);c.calls[0].ok(source);await p;assert.equal(c.sent.length,1,'Navigation must discard old-owner snapshot');
