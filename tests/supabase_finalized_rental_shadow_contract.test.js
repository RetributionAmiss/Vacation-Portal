...
// Match the browser's lexical DATA binding while allowing preview telemetry.
...
function runLexicalDataScenario(source){
  ...
  context.top={postMessage(message){if(message.type==='vacation-portal-supabase-rentals-request') sent.push(message);}};
...
assert.strictEqual(runLexicalDataScenario(client),true,'Loaded lexical DATA must reach MATCH without replacing visible rentals.');
console.log('PASS preview lexical DATA regression');
