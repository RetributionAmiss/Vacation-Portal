...
assert(
  ...
  'Itinerary comment lifecycle behavior must remain after read/write/concurrency bridges.'
);
...
assert(
  ...
  'The Itinerary comment lifecycle must ignore unrelated shared Supabase responses and must not take ownership of the authoritative comment reader.'
);
assert(
  ...
  'Itinerary comment freshness must remain tied to the established planner-social settle path.'
);
assert(
  ...
  'The accepted UI must still expose removal to the comment owner and authenticated organizer membership.'
);
assert(
  ...
  'The shared comment-section UI must continue supporting Itinerary and Meals removal controls.'
);
assert(
  ...
  'Itinerary removal must remain on its accepted Supabase-primary/fallback-mirrored path; the later Meals authority layer may wrap the transitional Meals branch.'
);
assert(
  ...
  'Itinerary deletion must keep updating and verifying its Sheets rollback section without undoing Supabase success.'
);
assert(
  ...
  'Comment removal must keep using the themed portal confirmation instead of the browser/iframe confirm dialog.'
);
assert(
...
assert(
  ...
  'Supabase Itinerary comment status/delete must continue using authenticated membership and version-checked RLS deletion.'
);
assert(
  ...
  'Itinerary comment deletion must fail closed through RLS without privileged browser credentials.'
);
...
assert(
  ...
  'Sheets rollback deletion must authorize owners through traveler binding and other-comment deletion through centralized organizer assertion.'
);
assert(
